...
var (
	loc = localization{
		m: map[string]string{
			...
			"k_card":       "Придумайте движение которое вы все будете делать перед каждым разом, как будете пить дальше. Придумывает тот игрок, чей сейчас ход. Тот, кто забыл сделать движение перед тем как выпить - пьет еще раз. \nЕсли у вас уже есть жест, и король попадается 2ой, 3ий, 4ый раз - жесты идут друг за другом. Да, к концу игры, для того что бы выпить придется делать по 4 движения :)",
