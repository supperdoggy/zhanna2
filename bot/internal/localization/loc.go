...
var (
	loc = localization{
		m: map[string]string{
			...
			"den4ik_logo":  "логотип игры",
			"den4ik_rules": "правила игры, перед началом надо ознакомиться, играть рекомендуется с одного телефона и просто по очереди нажимать на команду",
		},
	}
	// for danet
